...
public class Worker extends SocketClient implements Runnable {
	...
	protected URLClassLoader myClassPath;
	// TODO job queue , URLClassLoader fix, 
    ...
    public Worker(String[] args) {
    	...
    	Utils.writeInt(out, Utils.W2M_WP2P_PORT, Utils.BASE_WP2P_PORT+id); 
    ...
    public void sendFilesList(File path) {
    	if (path.isDirectory()) {
        	...
        	Utils.writeFilenames(out, Utils.M2W_REQ_LIST_OKAY, names);
    	}
    	else
    		Utils.writeFilenames(out, Utils.M2W_REQ_LIST_OKAY, new String[0]);
    }
    ...
    public void receive(int command) throws IOException {
		...
		case Utils.M2W_MR_UPLOAD:
			...
			Mapper<?, ?, ?> mr = loadMRFile(Utils.readFile(in, basePath + File.separator));
			if (mr != null) {
				Utils.writeCommand(out, Utils.ACK);  // notify master, next message received is the file listing
			...
			break;	
		case Utils.M2W_FILE:
			Utils.readFile(in, basePath + File.separator);
			break;
